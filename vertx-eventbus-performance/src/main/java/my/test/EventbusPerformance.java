...
public class EventbusPerformance extends Verticle {
	...
	public void start() throws Exception {
		container.deployVerticle("my.test.MessagePingPong", new JsonObject(), INSTANCES, new Handler<String>() {
			@Override
			public void handle(String event) {
				System.out.println(event);
				vertx.eventBus().registerHandler(MONITOR, new Handler<Message<JsonObject>>() {
					@Override
					...
					public void handle(AsyncResult<Void> event) {
						...
						for(int i = 0; i < 4 * INSTANCES ; i++){
							vertx.eventBus().send(PROCESSOR, new JsonObject().putNumber("recv_nr", 0));
						}
					}
				});
			}
	...
	public void monitor(JsonObject o){
		System.out.println();
		System.out.println(o.getNumber("name")+" 1000000 in "+o.getNumber("time")+"ms (total "+o.getNumber("recv_nr")+")");
